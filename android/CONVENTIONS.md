...
### Шторка-ФОРМА (несколько полей) — не должна прыгать от клавиатуры (2026-09-24)

Жалоба владельца: формы «дёргаются». Замер на эмуляторе: шторка ниже экрана при открытии клавиатуры
вырастала вверх на ~380 px и падала обратно при каждом переходе между текстовым полем и нетекстовым
контролом (чип, пикер грядки, выбор из списка). Правила для шторок с полями ввода (посадка,
редактирование посадки, урожай, пакетик семян):

1. **Сразу во всю высоту** — `.fillMaxHeight()` у колонки (до `verticalScroll`). Тогда клавиатура
   сжимает прокручиваемую область, а шторка стоит на месте.
2. **Дата — только `ui/common/DateField`** (тап → `DatePickerDialog`, поле `enabled = false`, фокус не
   берёт). Не вводить дату текстом «ДД.ММ.ГГГГ».
3. **Правильный тип клавиатуры**: количество — `KeyboardType.Number`, вес — `Decimal`, «12.2027» —
   `Decimal` (у `Number` нет точки). Иначе клавиатура меняет высоту при переходе между полями.
4. **Выбор из списка / чип / пикер снимает фокус** — `focusManager.clearFocus()`.
   ⚠️ `LocalFocusManager.current` брать **внутри** содержимого `ModalBottomSheet { … }`: шторка —
   отдельное окно со своим фокусом, менеджер, взятый снаружи, до её полей не дотягивается (так и
   было в первой версии фикса — эмулятор поймал).
...
| `isGuest()` / `setGuest(b)` | Гостевой режим (POST /auth/guest); хранится рядом с токеном |
| `getGuestDeviceId(): String` | Случайный UUID устройства — «пароль» гостя, создаётся при первом вызове; стирается в `logout()` |
| `incGuestActions()` / `isGuestNudgeDue()` / `closeGuestNudge()` | Шторка «Сохраните записи» после 3-го действия гостя (один раз) |
| `getHiddenSeasonWorks()` / `hideSeasonWork(key)` | «Сделано/Не актуально» в блоке «На этой неделе», ключ `id:год` |
...
> **Полный runbook деплоя (backend + веб-SPA + лендинг) и доступ к VPS — `docs/DEPLOY.md`.**
> Ключевое: `ssh hetzner` работает **только через PowerShell-инструмент** (ключ в Windows ssh-agent;
> Bash-инструмент доступа к ключу не имеет). В удалённых командах — **только одинарные кавычки, без `"`/`<`/`>`**
> (PowerShell их коверкает). Веб-SPA и лендинг — отдельные каталоги (`/var/www/dacha-web`, `/var/www/dacha-landing`).
...
- **`services/emailService.js`** — несколько драйверов, выбор по env (приоритет): **Brevo (HTTP API)**
  если `BREVO_API_KEY` → **Unisender Go (HTTP API)** если `UNISENDER_GO_API_KEY` → **SMTP** (nodemailer)
  если `SMTP_HOST` → иначе почта
...
- ⚠️ **Hetzner режет исходящий SMTP** (порты 25/465/587 — TLS глохнет). Поэтому на этом VPS почта идёт
  **только по HTTP API (443)**. SMTP-драйвер оставлен как фолбэк для других хостингов.
- **Brevo** (выбран как основной): `POST https://api.brevo.com/v3/smtp/email`, заголовок `api-key`,
  тело `{ sender:{name,email}, to:[{email}], subject, htmlContent, textContent }`, успех = 201.
  Подтверждение домена — через CNAME/TXT (reg.ru это умеет), бесплатный тариф. `from` = `SMTP_FROM`.
- **Unisender Go** (альтернатива, НЕ выбран): требует NS-делегирования трекинг-домена (reg.ru не умеет
  добавлять NS для поддомена → ошибка 229 `tracking domain required`). Драйвер оставлен в коде.
