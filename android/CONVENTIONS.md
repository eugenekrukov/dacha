...
Перед написанием или изменением любой `data class` модели (Models.kt) — открыть соответствующий роут в `backend/src/routes/` и сверить **имена полей**.
...
| 2026-05-31 | Post-MVP | TokenStorage: +saveClimateZone/getClimateZone/hasGarden. PlantingsRepository: +updateInfo(plantingId, UpdatePlantingInfoRequest). Planting: +quantity, conditions, lastActionAt. Новый UpdatePlantingInfoRequest. Таблица action_logs (не actions). |
