...
## Версии и релизы
- **vc18 / 1.0.15 — собран 2026-09-24, ждёт загрузки** (Google Play + RuStore одной версией): гостевой
  режим (`GuestStartScreen`, `GuestAuthenticator` — продление токена гостя на 401, шторка
  «Сохраните записи»), блок «На этой неделе» (`ui/today/SeasonWorksCard.kt`), подписка в
  `AccountSection`, `ui/common/DateField` (дата календарём), шторки-формы во весь экран — правила в
  `android/CONVENTIONS.md` («Шторка-ФОРМА»). В сторах сейчас vc17 / 1.0.14 (17.09).
- Ниже — история до vc12 (не обновлялась между vc12 и vc17, актуальное — `summary.md`).
- В магазинах (Google Play и RuStore одновременно): **vc11 / 1.0.8** — тёмная тема, «Мои семена»,
  советы дня по культуре, фиксы шторок и дат на «Урожае», просмотр фото пакетика (модерация
  пройдена 2026-08-03; для Google Play это первый production-релиз после closed testing).
- Предыдущая **vc10 / 1.0.7** — free+premium вместо триала, напоминание об осмотре посадки.
- Следующая **vc12 / 1.0.9** — не собиралась. В коде ждут мелочи с правок задач дня 2026-08-03:
  `tasksHidden` + строка «Ещё задач: N», `maxLines` описания задачи 1→2. См. [[02 Backend]].
