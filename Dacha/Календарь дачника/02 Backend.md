...
## Тесты
**vitest**, не jest. Запуск: `npm test`. 493/493 на момент последнего обновления.
См. `TESTING.md` в репо.

## Полив — один расчёт на три точки (2026-08-03)
`utils/todayLogic.wateringStatus(planting, weather, lastWatered, lastRain, today)` — источник
правды для `GET /today`, `careRemindersJob` (пуш в 09:00) и `recommendations.js`. Учитывает
теплицу (×0.8), испарение (жара/сухость/ветер, пол множителя 0.5), частоту и норму по стадии из
`crops.watering_details`, зачёт прошедшего ливня (≥5 мм) вместо полива и отмену по прогнозу
(нужны вероятность ≥70% И объём ≥3 мм; в теплице дождь не считается вовсе).

Собственные формулы в вызывающих запрещены: до этой сессии `recommendations.js` считал теплицу
как ×1.3 — ровно наоборот к ×0.8 в задачах, и участок жил по двум взаимоисключающим правилам.
Там же почищено: заморозки предупреждают за 1–2 дня по `forecast_json`, снимок старше 24 ч в
задачах не участвует, сортировка учитывает просрочку (`urgency()`), `tasks_total` считается до
среза. Детали — `session-note.md` (2026-08-03).

## Недавно добавлено
- **2026-09-23 — гостевой режим**: миграция 092, `POST /auth/guest` (идемпотентен по `guest_device_id`),
  `POST /auth/guest/claim` (email/пароль в ту же строку `users`), декоратор `requireAccount`
  (403 `account_required`) на оплате/промокоде/смене пароля и email. Тесты — `guest.test.js`.
- **2026-09-23 — «На этой неделе»**: `GET /season-works`, контент `src/data/seasonalWorks.js` (80 работ,
  окна от фактического сезона участка, `zones`, crop-списки), тест покрытия недель по зонам.
- **2026-09-24** — миграция 093: рассада (`transplant_days = 30`) у кабачка, тыквы, патиссона.
