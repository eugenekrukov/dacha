...
# П4 «Аккаунт и безопасность» (2026-06-15):
✓ PATCH /auth/password — верный текущий → 200; неверный → 401; new<6 → 400; без токена → 401
✓ POST /auth/change-email — верный пароль+свободный email → 200; неверный пароль → 401; занятый → 409; невалидный email → 400
✓ POST /auth/confirm-email-change — валидный код+свободный pending → 200; неверный код → 400; pending занят → 409
✓ DELETE /auth/me — верный пароль → 200 (анонимизирует payments, удаляет users); неверный → 401; без токена → 401
```

> Фактический бэкенд-сьют (2026-06-16): **264 теста** в 20 файлах. Запуск — `npx vitest run`.
> Документ ниже — изначальный план структуры тестов; фактическая реализация шире (billing, promo,
> guide, fcmService, careRemindersJob, access, push-tokens и др.).
