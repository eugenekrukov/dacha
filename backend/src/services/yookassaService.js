...
// (3) подтверждение — вебхук payment.succeeded; (4) чек 54-ФЗ — объект receipt.
...
// Чек. Режим — env YOOKASSA_RECEIPT_MODE:
//   'on' (по умолчанию) — передаём объект receipt с email покупателя и позицией услуги.
//       ЮKassa формирует чек и шлёт его на customer.email. Для САМОЗАНЯТОГО при подключённой
//       в кабинете интеграции с «Мой налог» ЮKassa также регистрирует доход в ФНС и формирует
//       чек НПД (422-ФЗ). vat_code=1 = «НДС не облагается» — корректно для самозанятого (без НДС).
//   'off' — receipt НЕ передаём. Включать ТОЛЬКО если кабинет ЮKassa формирует чек самозанятого
//       полностью сам и наш объект не нужен. ⚠️ тогда ЮKassa не получит email покупателя из платежа.
// Самозанятые освобождены от ККТ (54-ФЗ); их чек — НПД по 422-ФЗ, его делает интеграция «Мой налог».
