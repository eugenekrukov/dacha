...
FAQ:
В: Можно ли поливать днём?
О: Лучше нет — капли на листьях
под солнцем обжигают их.

В: Как часто поливать в жару?
О: Ориентируйтесь на почву, а не на календарь.
...
Теги: #подкормка`
  ...
  it('разбирает посты: дата (МСК), заголовок, тело, FAQ, telegram-версия, теги, картинка', () => {
    ...
    expect(posts[0]).toEqual({
      ...
      faq: [
        { q: 'Можно ли поливать днём?', a: 'Лучше нет — капли на листьях под солнцем обжигают их.' },
    ...
    expect(posts[1]).toMatchObject({ title: 'Подкормка', tags: '#подкормка', image: null, telegramBody: null, faq: [] })
