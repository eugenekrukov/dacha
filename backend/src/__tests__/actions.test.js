...
// поэтому кейсы ниже проверяют логику журнала, а не гейт (гейт — отдельным describe).
...
describe('POST /actions', () => {
  ...
  it('подписчик пишет в журнал любой своей посадки, даже вне free-набора', async () => {
    const app = await buildApp({
      query: async (sql) => {
        const ft = freeTierQuery(sql, { subscribed: true, freeIds: [7, 8, 9] })
        if (ft) return ft
        if (sql.includes('INSERT INTO action_logs')) return { rows: [ACTION] }
        return { rows: [{ id: 1, stage: 'growing' }] }
      },
    })
    const token = makeToken(app)
  ...
  it('завершённая посадка (stage=done) остаётся редактируемой — архив не блокируем', async () => {
    const app = await buildApp({
      query: async (sql) => {
        const ft = freeTierQuery(sql, { freeIds: [7, 8, 9] })
        if (ft) return ft
        if (sql.includes('INSERT INTO action_logs')) return { rows: [ACTION] }
        return { rows: [{ id: 1, stage: 'done' }] }
      },
    })
    const token = makeToken(app)
