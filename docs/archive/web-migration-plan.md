...
> **Не сделано:** сброс пароля (бэкенд готов), Web Push, геолокация (нет reverse-geocode). Деплой веба —
> пересборка на VPS + `cp` в `/var/www/dacha-web`. (Календарь — **готов**, перенесён 2026-06-14, см. Фаза 3 п.5.)
...
- [x] 5. **Календарь — ПЕРЕНЕСЁН из Android** (2026-06-14). Клиентский порт, без нового бэкенда.
      - **Данные**: `GET /plantings`, `GET /crops`, `GET /today?garden_id` (его `tasks`), грузятся параллельно.
        Reminders и snooze в вебе пока не поддерживаются → не учитываются.
      - **Логика**: `buildCalendarEvents` в `web/src/api/schedule.ts` (порт `CalendarViewModel.buildEvents`):
        `Record<ISODate, CalendarEvent[]>`, типы `sowing|harvest|watering|care` + проброс типов задач дня;
        care-задачи — по `care_tasks` культуры; для многолетников сезонный сброс (`effectivePlanted`,
        зеркало backend `effectivePlantedAt`); горизонт 60 дней; завершённые посадки пропускаются.
      - **UI**: `web/src/screens/CalendarScreen.tsx` — сетка месяца (Пн-based, пред./след. месяц, точки на
        днях с событиями, выделение сегодня/выбранного) + список событий дня. Пункт «Календарь» в меню
        «Информация» (Layout), маршрут `/calendar`.
...
> **Осталось по Фазе 3 (вторичное)**: экран сброса пароля (двухшаговый,
