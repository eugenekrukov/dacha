...
|---|---|
| **App access** | Приложение **требует вход** → дать ревьюверу тестовый аккаунт: `demo@dacha.ru` / `demo1234` (прод, `store=gplay`, триал 30 дн — полный доступ без paywall, участок Краснодар с данными). Без этого отклонят (не смогут пройти дальше логина). |
