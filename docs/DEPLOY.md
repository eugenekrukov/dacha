...
⚠️⚠️ **Два хоста, ДВА nginx-конфига — `/etc/nginx/sites-available/dacha` И `.../calendacha`.**
`calendacha.ru` — не просто редирект на `dacha.studio1008.com`: у него свой `location /app/ {
alias /var/www/dacha-web/; }`, физически та же папка, что и на `dacha.studio1008.com`. Пользователи
реально заходят в приложение и через `calendacha.ru/app/`, не только через `dacha.studio1008.com/app/`.
**Любую точечную nginx-правку под `/app/` (exact-match location, no-cache на index.html и т.п.)
нужно вносить в ОБА конфига**, иначе баг воспроизводится только на одном домене — грабли
2026-09-03: `/blog/feed`-фикс сначала попал только в `dacha`, `calendacha` получил 404 на том же
роуте отдельным заходом позже, пока диагностировали жалобу пользователя (он сидел на `calendacha.ru`,
а все проверки шли на `dacha.studio1008.com` — молчаливое несовпадение стоило половины сессии).

⚠️ **Браузеры кэшируют 301 «намертво»** — тот же грабли ударили в проде: пока редирект не был
исправлен (первые минуты после деплоя), у части посетивших `/reference`/«Сегодня» браузер
запомнил `301 → calendacha.ru/blog/feed` для конкретного URL и после исправления сервера
продолжал молча ходить на несуществующую страницу (JS ловит ошибку и тихо показывает «Статей
пока нет» — рассчитанное поведение для реального отсутствия статей, тут — маскирует баг
кэша). Обычный Ctrl+Shift+R (hard refresh) **не всегда** сбрасывает уже закэшированный редирект
для fetch-запросов, отправленных JS уже после загрузки страницы — нужна полная очистка кэша сайта
(«Очистить данные о просмотренных страницах» → «Файлы cookie и другие данные сайтов» +
«Кэшированные изображения и файлы», диапазон «Всё время» либо конкретно для домена) или
приватное окно для проверки. Добавлен `add_header Cache-Control "no-store" always;` в
`location = /blog/feed` — не лечит уже закэшированное у пользователей, но не даёт повториться
при следующих временных сбоях этого роута.

**Правила написания статей (блог — основной канал с 2026-09; длина, структура под запросы,
FAQ, слаг, Schema.org) — в [`docs/blog-content-guide.md`](blog-content-guide.md).** Ниже — только
техника `FAQ:`.

**При подготовке каждого нового поста секция `FAQ:` обязательна** (правило владельца,
2026-08-13; с 2026-09 — 3–5 пар). Пары `В:`/`О:` отделены пустой строкой, ставятся в тексте
после основного тела (перед `Telegram:`, если он есть):
...
- **2026-09-24** — миграция **093** (`transplant_days = 30` у кабачка/тыквы/патиссона) + веб (выбор
  ...
  бэкенд не перезапускался (изменились только данные; `/crops` без кэша — проверено API).
- **2026-09-23** — гостевой режим и «На этой неделе»: бэкап `/root/users-before-092-20260923.sql`,
  миграция **092** (`users.email`/`password_hash` nullable, `is_guest`, `guest_device_id`),
  `pm2 restart`, веб пересобран, `landing/privacy.html` и `account-deletion.html` скопированы в
  `/var/www/dacha-landing` (бэкапы `/root/*.bak-20260923.html`). Вечером — фикс даты статей на вебе.
  Проверено вживую: гость → участок → «Сегодня» → «Аккаунт» → оплата ведёт на регистрацию → удаление.
