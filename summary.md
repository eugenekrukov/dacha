...
**После публикации — не забыть:**
- RuStore Console: перенести обновлённый FAQ карточки (8 вопросов, `docs/aso-rustore.md` §FAQ —
  добавлены «с компьютера» и «без интернета», из ответов про тариф убраны явные цифры лимита),
  залить новые скриншоты.
- Google Play: лимит **8** скриншотов на телефон из имеющихся 10 (кандидаты на снятие — №5 грядки
  и №10 тёмная тема, см. `session-note.md` 2026-07-30 (2)); в наборе нет кадра с «Моими семенами».
- Проверить на живом релизе регистрацию → оплату → чек НПД (последний E2E — 19.06).
