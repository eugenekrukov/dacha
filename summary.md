...
**📤 vc11/1.0.8 СОБРАН И ВЫЛОЖЕН в Google Play и RuStore (2026-07-30), ждёт модерации.**
Google Play — первый production-релиз после closed testing (ревью личного аккаунта обычно дольше
обычного). Что вошло: тёмная тема, «Мои семена», советы дня по культуре, фиксы шторок и тёмной
темы, просмотр фото пакетика, ссылка на веб-версию. Тексты карточек и «Что нового» —
`docs/aso-rustore.md` + `session-note.md` (2026-07-30 (2)).
⚠️ Сверить, что собрано из `main` не раньше `933e1a4` — там последняя из пяти правок on-device QA.
