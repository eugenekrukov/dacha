...
**⏳ Pending deploy** (запушено в `origin/main`, НЕ выложено — по просьбе подождать):
- Коммиты `b8641ef` (контактная почта → `dacha@studio1008.com`) + `9ffa82d` (фикс шапки лендинга в одну строку).
- Выкладка: VPS `git reset --hard origin/main` → `cp landing/index.html /var/www/dacha-landing/` + пересборка веба (`/var/www/dacha-web`).
- ⚠️ Завести почтовый ящик `dacha@studio1008.com`.
