...
**⚠️ ПЕРЕСОБРАТЬ AAB перед публикацией vc11** (2026-07-30, `020c600`): on-device QA семян нашёл
дефект шторок — `ModalBottomSheet` без `skipPartiallyExpanded` открывается на половину экрана и
обрезает нижние поля/кнопку «Сохранить», без `verticalScroll` до них не добраться из-под
клавиатуры. Исправлены `SeedsScreen` и `AddHarvestSheet` («Убрать урожай» — дефект был и в проде).
Каркас зафиксирован в `android/CONVENTIONS.md`. Если AAB собран до этого коммита — собрать заново.
