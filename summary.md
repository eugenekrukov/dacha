...
- [ ] **Написать тесты** — структура описана в `TESTING.md`. Приоритет: бэкенд unit-тесты (todayLogic, weatherService), потом интеграционные (auth, today), потом Android ViewModels. Инструменты: Vitest + Supertest (backend), MockK + Turbine (Android).
