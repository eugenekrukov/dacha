...
### ⏳ TO-DO следующей сессии (приоритет ↓) — обновлено 2026-06-10

**Состояние git:** `main` = `d0778e2` (E5 влит+запушен). Пуш-фича FCM закоммичена на ветке
**`feature/push-fcm`** (НЕ влита в main, НЕ запушена, НЕ задеплоена) — начать с неё.

1. **Пуши FCM — довести до прода** (код готов: бэкенд+Android, см. блок «Пуши → FCM» в session-note):
   - [ ] `feature/push-fcm` → ff-merge в `main` + push (нужен «ок» пользователя).
   - [ ] Деплой бэкенда: `fetch+reset` → `cd backend && npm install` (firebase-admin!) → миграция
     `026_push_provider.sql` → `pm2 restart`.
   - [ ] **Сервисный аккаунт Firebase** на VPS (`/var/www/dacha-api/secrets/...json`) +
     `FCM_SERVICE_ACCOUNT_PATH` в `backend/.env` → `pm2 restart`. (Пользователь генерит ключ в Firebase.)
   - [ ] Собрать `:app:assembleGplayDebug`, поставить на Samsung A55 → проверить регистрацию FCM-токена
     (`provider=fcm`, длинный) и доставку тест-пуша.
2. **E5 — деплой store-гейта + проверка рекламы:**
   - [ ] ⚠️ Проверить/применить миграцию **`025_user_store.sql`** на VPS (статус деплоя E5 не подтверждён).
   - [ ] Проверить ad-сборку на устройстве: баннер РСЯ снизу, интерстишл на ~6-е переключение, нет Paywall,
     доступ не блокируется 402 после триала. Боевые ID РСЯ уже вписаны (`R-M-19420797-1/2`).
3. **E4 — финал на устройстве:**
   - [ ] Пересобрать `rustore`-APK, проверить оплату ЮKassa с устройства (Custom Tab → оплата → доступ),
     отключение автопродления (его нет — разовая оплата), промокоды/триал.
   - [ ] Вернуть тестовые 299 ₽ в кабинете ЮKassa. Сменить пароль тест-аккаунта `e-krukov@ya.ru` (светился).
4. **Гигиена git:** удалить влитые стейл-ветки `feature/email-brevo|email-unisender|email-verification|
   notif-permission` (+ `feature/push-fcm` после merge).
5. **E3 (давний долг):** Android unit-тесты не запускаются (баг тулчейна AGP 9). Проверка — `compile*Kotlin`.

**Бэкенд-тесты:** 210/210. **Android:** все флейворы `compile`/`assembleGplayDebug` BUILD SUCCESSFUL.
