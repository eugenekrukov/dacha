...
- **Бэкенд-тесты**: 456/456 (`npm test` → vitest run; **НЕ** jest).
...
**🆕 Инвентарь семян (РЕАЛИЗОВАН 2026-07-29, ⚠️ НЕ ЗАДЕПЛОЕН):**
...
- **Деплой**: `git push` → на VPS `fetch && reset --hard origin/main` → `npm run migrate`
  (миграция 060) → `pm2 restart dacha-api`; веб — `npm run build` → `/var/www/dacha-web`.
  До деплоя бэкенда экран «Мои семена» на клиентах будет отдавать ошибку загрузки.
...
**✅ Опубликовано:** Google Play и RuStore оба на **vc10/1.0.7** (владелец подтвердил публикацию).
Собирает: **free+premium вместо 7-дневного триала** (главное изменение — см. «Монетизация» выше,
сессия 2026-07-18) + **напоминание об осмотре посадки** (`24aa0a3` — пункт меню посадки ставит
периодический локальный пуш через WorkManager, раз в день/2/3 дня, без изменений backend) + фикс
переноса лейбла «Календарь» в нижнем меню (`ae0f6c2`). Версия единая для Gplay и RuStore. Тексты
release notes — `session-note.md` (2026-07-18 (2)).

**🚧 Готовится к публикации: versionCode 10→11, versionName 1.0.7→1.0.8.** Google Play прошёл
closed testing и открыл доступ к production (2026-07-30). Состав шире, чем одна тёмная тема:
тёмная тема (тумблер «Система/Светлая/Тёмная») + контраст карточек, **инвентарь «Мои семена»**,
фикс ISO-дат на «Урожае», фикс подсказки севооборота. Готовый текст «Что нового» (452 символа,
общий для обоих магазинов) — `session-note.md` (2026-07-30 (2)). Сборку AAB и публикацию делает
владелец вручную. ⚠️ Android-часть «Моих семян» не проходила on-device QA — прогнать через
internal testing до раскатки. ⚠️ В Google Play лимит 8 скриншотов из имеющихся 10.
