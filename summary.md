...
**Групповые care-задачи — мульти-посадочное действие (следующая сессия):**
Сгруппированные задачи (несколько культур, `planting_id = null`) сейчас некликабельны.
Согласованное поведение: клик → листа действия, в заголовке список растений с крестиком
(мин. 1), одно действие пишется во все оставшиеся посадки. Нужно:
- Backend: добавить `planting_ids: number[]` и `crop_names_with_ids: {id,name}[]` в ответ
  grouped task из `formatTasks`/`buildTasks`
- Android: расширить `ActionLogBottomSheet` под мульти-посадочный режим
- Web: расширить `ActionLogSheet` аналогично
- Спек: `docs/superpowers/specs/2026-06-17-upcoming-tasks-section-design.md` (смежный контекст)
