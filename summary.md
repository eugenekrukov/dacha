...
- [x] Исправлен `today.js` — поля погоды приведены к реальной схеме БД
- [ ] Деплой на VPS + верификация `/weather` и `/recommendations` с реальными данными
- [ ] Android: WeatherRepository + реальная погода на TodayScreen + карточки рекомендаций
