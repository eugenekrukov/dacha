...
- **Грядки + севооборот («план участка»)**: бэкенд готов и ЗАДЕПЛОЕН (`garden_beds`, миграции 052/053,
  эндпоинты `GET/POST /gardens/:id/beds`, `PATCH/DELETE /beds/:id`, 3-летняя история севооборота), но
  **клиентского UI нет ни на Android, ни на вебе** — фича невидима пользователю. Нужен UI.
...
- **2.1** `logout()` на клиенте не зовёт `DachaApi.deletePushToken()` (метод мёртвый) — гигиена.
- **2.2** код samsung-флейвора в Android не удалён (флейвор не публикуется; не срочно).
