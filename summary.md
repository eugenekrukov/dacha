...
офлайн-режим «Сегодня» — то, что раньше было только в Google Play).

**🚧 Готовится к публикации (2026-07-11): versionCode 8→9, versionName 1.0.5→1.0.6**
(`android/app/build.gradle.kts`, поднято — сборку AAB и публикацию делает владелец вручную).
Собирает всё, что накопилось в `main` с vc8: перестройка «Профиля» (Аккаунт+Лента+Статистика) +
новый таб «Ещё» (Справочник/Болезни/Настройки), контраст hero «Сегодня» до WCAG AA (сессия
2026-07-08, см. `session-note.md`). Для Gplay (текущая vc8/1.0.5) это единственная новая волна.
Для RuStore (текущая vc7/1.0.4, отстаёт на волну грядок/севооборота/лунного календаря) релиз
кумулятивный — набор «что нового» ниже включает обе волны. Тексты release notes — там же
в session-note.md (2026-07-11).
