...
**✅ Групповые care-задачи — мульти-посадочное действие (РЕАЛИЗОВАНО 2026-06-17, НЕ задеплоено):**
...
- **⚠️ НЕ задеплоено**: нужен деплой backend (прод ещё на старом ответе `/today` без новых полей) +
  пересборка веба; Android AAB/APK публикует пользователь.
