...
**✅ Советы дня стали культуро-специфичными (2026-07-30, ⚠️ НЕ ЗАДЕПЛОЕН):** совет подписывается
именем культуры, но брался только из общего по стадии `STAGE_TIPS` — поэтому «окучьте картофель»
попадало на горох, а «трескаются корнеплоды» на укроп. Подключён культуро-специфичный слой
...
Приоритет: совет про культуру → фолбэк на общий пул. Пул заодно вычищен от культуро-специфичных
формулировок + тест-страж. Тесты 461/461. Миграций нет, деплой — вместе с семенами.
⚠️ **Данные заполнены у 32 культур из 55** — остальным доступен только общий пул (см. roadmap).
