...
**⚠️ Деплой**: миграция `022_sowing_method.sql` + код (`todayLogic`, `careRemindersJob`, `plantings`, `today`).
Смена владельца не нужна (ALTER+UPDATE существующих таблиц).

**Незакрытый шаг**: пересборка APK (поливы в расписании; способ посадки в шторках; календарь без
завершённых; «Взошло» убрано).
