...
**Дата последней сессии**: 2026-06-25

## Сессия 2026-06-25 (4) — фото справочника (11/16) + E3 исправлен (не задеплоено)
...
Файлы — в `web/public/media/guide/`, миграция `049_guide_images_3.sql` написана (UPDATE по slug,
идемпотентно). **НЕ задеплоено** — нужен прогон миграции на проде + ребилд веба, ждёт решения
владельца (вопрос задан, ответа не было). Без фото остались 5 (нет годного свободного кадра):
...
- Деплой фото (049) + Android build.gradle.kts фикса — commit+push → VPS `reset --hard` → веб
  `npm run build` → `sudo -u postgres psql -f 049_guide_images_3.sql` → не нужен `pm2 restart` (веб-статика).
- П1 Kindwise — ждём API-ключ от владельца.
