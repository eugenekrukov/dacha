...
- Деплоить эту сессию: backend (`plantings.js`) нужен `pm2 restart`, веб (`ActionLogSheet.tsx`,
  `AddPlantingForm.tsx`) — ребилд + копирование статики.
- Письмо-ремайндер для платного периода — нужно решение владельца, делать или не делать.
- П1 (3 слабых фото справочника: земляничный клещ, фузариоз донца, вертициллёзное увядание) — не
