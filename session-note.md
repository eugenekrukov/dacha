...
**Правки доков**: `summary.md` — добавлена карта документации (главный вход), harvest помечен задеплоенным, исправлена стале-запись про offline-today (был «в ветке/не задеплоен» → влит в main, backend в проде). Консолидация (см. ниже, коммит `048bd38`): 16 завершённых plan/spec → `docs/archive/`, в `docs/superpowers/` только активные (offline-today, unified-task-dismiss). Obsidian-ваулт: стале-копии `Корень репозитория/` заменены живыми hardlink'ами; зеркальные папки в `.gitignore`.
