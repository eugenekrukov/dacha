...
⏳ **PENDING DEPLOY (не задеплоено, по просьбе «не коммитить» доки тоже не закоммичены):**
коммиты `b8641ef`+`9ffa82d` запушены в origin/main, но НЕ выложены. Нужно: на VPS `git reset --hard
origin/main` → `cp landing/index.html /var/www/dacha-landing/index.html` (почта+шапка) + пересобрать
веб (`cd web && npm ci && npm run build` → копия в `/var/www/dacha-web`) ради смены mailto в Настройках.
