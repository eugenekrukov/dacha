...
### 7.1 Web — единый red для ошибок

Для ошибок/удаления на вебе (`web/src`) используется только Tailwind `red-600`/`red-700`/`red-100`
(так уже сделано в ~30 местах) — не заводить отдельный кастомный hex (`#D32F2F` и т.п.), даже если он
ближе к «эталонному» FrostRed из Android-палитры. Обнаружено и унифицировано 2026-09-06 (Impeccable audit).

### 7.2 Тёмная тема — осознанно не делаем (веб)

Веб-версия не поддерживает `prefers-color-scheme`/`dark:`-классы. Решение: аудитория (дачники 40+,
телефон при дневном свете на участке) не в приоритете тёмной темы, и веб — вторичная точка входа
к мобильному продукту. Не добавлять `dark:`-классы без отдельного запроса на фичу.
